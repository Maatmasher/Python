...
class SSHExecutor:
    ...
    def execute_commands(self, commands):
        ...
        try:
            for cmd in commands:
                result = {'command': cmd, 'output': '', 'error': '', 'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
                ...
                try:
                    ...
                    if exit_status != 0 or error:
                        self._log_error(f"Команда '{cmd}' завершилась с ошибкой (статус {exit_status}): {error}")
