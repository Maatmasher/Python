...
# Конфигурация пока всё под Windows, невзирая на первую строку.
...
# Сопоставление типов устройств с файлами команд, писать что то эдакое пока не осилил
TYPE_COMMANDS = {
    "POS": "command_POS.txt",
    "SCO": "command_SCO.txt",
...
def setup_logging():
    ...
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write("")  # Очищаем лог при запуске, не стал парится
...
async def run_plink(host, device_type):
    """Асинхронное выполнение команды через plink с учетом типа устройства"""
    ...
    try:
        ...
        try:
            ...
            if proc.returncode == 0:
                await log_message(f"{host} ({device_type}) Успешно\n{stdout}")
            else:
                await log_message(
                    f"{host} ({device_type}) Ошибка (код {proc.returncode})\n{stderr}"
                )
        except asyncio.TimeoutError:
            await log_message(f"{host} ({device_type}) Таймаут выполнения")
            proc.kill()
            await proc.communicate()
    except Exception as e:
        await log_message(f"{host} ({device_type}) Критическая ошибка: {str(e)}")
...
async def load_hosts():
    ...
    try:
        ...
        await log_message(f"Ошибка загрузки файла {CASH_IP_FILE}: {str(e)}")
        return []
...
async def main():
    ...
    if not hosts:
        await log_message("Нет хостов для обработки")
        return
