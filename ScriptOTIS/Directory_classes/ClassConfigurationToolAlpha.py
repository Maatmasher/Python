...
class ConfiguratorTool:
    ...
    def _execute_command(
        self, args: List[str], max_retries: int = 3
        ...
        while attempt < max_retries:
            ...
            try:
                ...
                # Если нет недоступных узлов, завершаем
    ...
    def get_nodes_from_file(
        self, filename: str = "server.txt", max_retries: int = 3
    ) -> Dict[str, List[Dict[str, str]]]:
        """Получить узлы из файла с повторами для недоступных"""
        filepath = self.config_dir / filename
        return self._execute_command(
            ["-ch", self.centrum_host, "-f", str(filepath)], max_retries
        )
    ...
    def update_servers(
        ...
        """Обновить серверы с повторами для недоступных"""
        filepath = self.config_dir / filename
        ...
        return self._execute_command(args, max_retries)
    ...
    def update_cash_devices(
        ...
        """Обновить кассовые устройства с повторами для недоступных"""
        filepath = self.config_dir / filename
        args = [
            "-ch",
            self.centrum_host,
...
if __name__ == "__main__":
    try:
        ...
        # 2. Обновить кассы с повторами
        cash_update = configurator.update_cash_devices(
            cash_type="POS", version="10.4.14.14", max_retries=3
        )
