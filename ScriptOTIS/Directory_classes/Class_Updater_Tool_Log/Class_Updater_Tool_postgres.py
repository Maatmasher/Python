...
# Настройки подключения к tools
DB_HOST = "10.21.11.201"
DB_PORT = 5432
DB_NAME = "GD"
DB_USER = "postgres"
DB_TABLE = "tpl_muk"  # Название таблицы с информацией об узлах
...
class UnifiedServerUpdater:
    ...
    def _init_db_password(self) -> str:
        ...
        try:
            ...
            if self._test_db_connection(db_password):
                keyring.set_password(DB_SERVICE_NAME, DB_USER, db_password)
                logger.info("Пароль БД успешно сохранён в keyring")
                return db_password
            else:
                raise ValueError("Неверный пароль для базы данных")

        except Exception as e:
            logger.error(f"Ошибка при работе с keyring для БД: {str(e)}")
    ...
    def _get_nodes_from_database(self) -> Dict[str, Dict[str, Optional[str]]]:
        ...
        logger.info("Получение информации об узлах из базы данных PostgreSQL")
        ...
        try:
            ...
            logger.debug(f"Успешное подключение к БД {DB_NAME} на {DB_HOST}:{DB_PORT}")
            ...
            with conn.cursor(cursor_factory=DictCursor) as cursor:
                ...
                logger.debug(f"Выполнение запроса: {query.as_string(conn)}")
    ...
    def _convert_db_rows_to_nodes_format(
        ...
        logger.debug(f"Конвертация {len(rows)} строк из БД в формат узлов")
        ...
        for row in rows:
            try:
                ...
                if not node_key:
                    logger.warning(f"Пропуск строки без ip и tp: {row_data}")
                    continue
                ...
                logger.error(f"Ошибка при обработке строки БД: {row_data}, ошибка: {str(e)}")  # type: ignore
                continue

        logger.info(f"Успешно сконвертировано {len(nodes_dict)} узлов из БД")
        return nodes_dict
