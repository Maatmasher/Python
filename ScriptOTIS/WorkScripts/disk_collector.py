...
class DiskInfoCollector:
    def __init__(self, username, password=None, max_workers=50):
        ...
        if not self.password:
            self.password = getpass.getpass(f"Введите пароль для пользователя {self.username}: ")
    ...
    def load_hosts_from_json(self, json_filename):
        ...
        try:
            if not os.path.exists(json_file_path):
                self.logger.error(f"Файл {json_filename} не найден в директории {self.script_dir}")
                return []
            ...
            if output and not output.startswith('ERROR'):
                ...
                if len(parts) >= 5:
                    ...
                    if '/dev/sda1' in parts[0]:
                        ...
                        self.logger.info(f"✓ {host_ip}: успешно собрана информация ({execution_time}s)")
                    else:
                        self._log_error(host_ip, "Filesystem не содержит /dev/sda1", host_info)
                else:
...
def load_config(config_filename="config.json"):
    ...
    try:
        ...
        print(f"Файл конфигурации {config_filename} не найден, используются значения по умолчанию")
        return {
            "ssh_username": "tc",
            "max_workers": 50,
            "timeout": 10,
